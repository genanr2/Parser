...
//	char str[64];
	...
	time(&ltimeStart);
	CharToOem("Не вводите, пожалуйста, свои строки\n",str);
	fprintf(stderr,str);

	main3(argc,argv);
	CharToOem("Не вводите, пожалуйста, свои строки2\n",str);
	fprintf(stderr,str);
...
//		pXL->Visible=VARIANT_TRUE;
	  ...
	  WorkbooksPtr pBooks=pXL->Workbooks;
		...
		strcat(str,"\\all.xls");
		_WorkbookPtr pBook;
		try{pBook=pBooks->Open(str);}catch(_com_error&e){dump_com_error(e);}
...
//		str=buffer;
					...
					char regN[20];
					strcpy(fileName,"C");
					if(bb->regionNo<=9)sprintf(regN,"00%d",bb->regionNo);
					else if(bb->regionNo<=99)sprintf(regN,"0%d",bb->regionNo);
					else if(bb->regionNo<=999)sprintf(regN,"%d",bb->regionNo);
//					else sprintf(regN,"%d",bb->regionNo);

					strcat(fileName,regN);
					sprintf(year,"%d",bb->year);sprintf(regN,"%s",year[strlen(year)-2]);
...
/*
		try{pBook->Saved=VARIANT_TRUE;pBook->Close();}catch(_com_error&e){dump_com_error(e);}
//		try{HRESULT res=pXL->Quit();}catch(_com_error&e){dump_com_error(e);}
